...
import { Play, Eye, Heart, MessageSquare, Film } from 'lucide-react'
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard icon={Eye} iconTone="blue" label="조회수" value={twViews.toLocaleString()} delta={twViews - pwViews} prevValue={pwViews} delay={0} />
          <MetricCard icon={Heart} iconTone="pink" label="좋아요" value={twLikes.toLocaleString()} delta={twLikes - pwLikes} prevValue={pwLikes} delay={0.05} />
          <MetricCard icon={MessageSquare} iconTone="green" label="댓글" value={twComments.toLocaleString()} delta={twComments - pwComments} prevValue={pwComments} delay={0.1} />
          <MetricCard icon={Film} iconTone="purple" label="영상 수" value={`${thisWeekVideos.length}`} suffix="개" delta={thisWeekVideos.length - pwIds.size} prevValue={pwIds.size} delay={0.15} />
        </div>
      </div>
