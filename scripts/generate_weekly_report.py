...
1. **이번 주 TOP 3 영상 공통점** (주제, 제목 스타일, 길이 등)
2. **성과 좋은 주제 패턴** (어떤 법률 주제가 조회수 높은지, 표로 정리)
3. **성과 좋은 제목 패턴** (질문형/공분형/반전형 등 카테고리별 분석)
4. **부진 영상 분석** (왜 안 됐는지, 주제·타이밍·제목 중 원인 분석)
5. **숏폼 vs 롱폼 성과 비교** (유형별 평균 조회수, 어느 쪽이 효과적인지)
6. **핵심 인사이트** (🔥/❄️ 이모지로 시작하는 한 줄 인사이트 3~5개)
